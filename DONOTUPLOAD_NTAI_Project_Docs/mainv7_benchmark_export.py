# Description: This script is used to generate a custom report for a stock and a benchmark. The user can select the graphs and tables they want to display in the report. The report can be exported to an HTML file or a CSV file. The script also generates a snapshot of the stock's performance using the QuantStats library and saves it to an HTML file. The user can view the snapshot in the Streamlit app. The script uses the AgGrid component to display tables in the Streamlit app. The script also uses the Streamlit download_button function to create download buttons for exporting the report to HTML and CSV files. The script uses the Streamlit components.html function to display the snapshot HTML file in the Streamlit app. The name of the app is MarketMomentum. The homepage reads Welcome to MarketMomentum. The app has a sidebar with the following widgets: a text input for the stock symbol, a checkbox for including a benchmark, a text input for the benchmark symbol, date input widgets for the start and end dates, and buttons for Snapshot and Custom Report. The app displays the selected graphs and tables in the main section based on the user's input. The app also has download buttons for exporting the report to HTML and CSV files. The app displays the snapshot HTML file in the Streamlit app.
